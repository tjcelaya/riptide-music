...
$app->get('/dsearch/:params+', function($params) use ($discogs, $sqlConnection) {
  $artist = $params[0];
...
$app->post('/dimport/:discogsID', function ($discogsID) use ($app, $sqlConnection, $discogs) {
    ...
    array_walk($albumInfo['genre'],
      function ($g) use ($albumCheck, $sqlConnection) {
        ...
        $albumTagsInsert = 'INSERT INTO AlbumTags '.
            ...
            'VALUES '.
            "({$albumCheck[0]['albumID']},'$g');";
        mysqli_real_query($sqlConnection, $albumTagsInsert);
    });

  }
  
  $app->redirect(
    "http://$_SERVER[HTTP_HOST]/~celaya/riptideMusic/album.php?id={$albumCheck[0]['albumID']}"
  );
});
